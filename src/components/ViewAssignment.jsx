...
//             <button className="mt-6 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-bold py-3 rounded-xl hover:scale-105 transition-all duration-300 shadow-lg">
//               Take assignment
//             </button>
//           </div>
//         </div>
...
const ViewAssignment = () => {
  ...
  const submitAssignment = async (e) => {
    e.preventDefault();
    if (!googleDocLink) {
      alert('Google Docs link must be provided!');
      return;
    }

    setLoading(true);
    try {
      const res = await fetch('http://localhost:5000/myattemps', {
        method: 'POST',
