...
//                   <span
//                     className={`text-xs font-medium px-2 py-1 rounded-full ${statusStyles[d.status] || "bg-gray-100 text-gray-800"
//                       }`}
//                   >
...
//                   By: <strong className="text-gray-900">
...
const Pending = () => {
  ...
  const [modalOpen, setModalOpen] = useState(false);
  const [selectedAssignment, setSelectedAssignment] = useState(null);
  const [loading, setLoading] = useState(true); // শুরুতে true
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => setData(res.data))
      .catch((err) => console.error("Error fetching data:", err))
      .finally(() => setLoading(false)); // ডাটা লোড হলে স্পিনার বন্ধ
  }, [user.accessToken]);
